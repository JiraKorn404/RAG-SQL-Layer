...
├── CLAUDE.md
├── README.md
├── pyproject.toml              # deps + [project.scripts] rag-sql-index, rag-sql-history
...
`AgentState` (in `agent/state.py`) carries: `thread_id`, `history` (earlier `Turn`s), `question`, `standalone_question`, `context` (retrieved docs), `similar_queries` (`PastQuery`s), `reasoning`, `sql`, `error`, `attempts`, `result` (columns + rows), `answer`, `turn_id`, `example_saved`.
